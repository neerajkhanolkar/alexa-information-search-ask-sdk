...
exports.handler = (event, context) => {
  ...
  alexa.appId = appId;
  alexa.registerHandlers(newSessionHandlers, fullInformationModeHandlers);
  alexa.execute();
}

const states = {
  FULL_INFORMATION_MODE: '_FULL_INFORMATION_MODE'
};
...
const newSessionHandlers = {
  ...
  'LaunchRequest': () => {
    this.attributes = [];
    this.handler.state = states.FULL_INFORMATION_MODE;
    this.response.speak('Welcome to person information Alexa skill')
                  .listen('Just ask using someones first name for their phone number or last name.');
